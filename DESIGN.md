...
Версия контракта: **1.0**. Статус: **спецификация для реализации**.

Документ фиксирует устройство системы, границы ответственности, форматы данных и правила взаимодействия. Дерево файлов, команды и тесты описывают целевую реализацию.

Основания: [техническое задание](materials/ТЗ_нефтекод.docx), [материалы](materials/README.md), [поэтапный план](IMPLEMENTATION_PLAN.md). ТЗ определяет обязательные требования, этот документ — технические контракты, план — порядок реализации. Изменение контракта сопровождается правкой этого документа и тестовых примеров в том же PR.
...
| Развёртывание | Одно локальное Python-приложение, Python 3.11 |
...
| UI | Streamlit; CLI для подготовки, обучения, оценки и воспроизведения |
...
В режиме `history` без модели последствий возвращается `abstain` с причиной `ACTION_MODEL_UNAVAILABLE`, но оценки состояния и качества сохраняются и показываются. Проверенный пример `hold` демонстрируется в `model_demo`; после проверки модели последствий он доступен и на истории. Отсутствие рекомендации не означает отсутствие прогноза.
...
flowchart TD
    ...
    UI[Streamlit или CLI] --> Orch[Оркестратор]
    State --> Orch
...
Диаграмма показывает поток данных и вызовов, а не сетевые сервисы. Обратная передача результата — обычный `return`.
...
- `agents/*` используют контракты, признаки и численные модели, но не Streamlit и не журнал.
...
Это целевое дерево. Файлы создаются по мере соответствующих этапов, без пустых заготовок «на будущее».
...
│   ├── ui.py                         # Streamlit; представление результатов
...
Backend не дублирует признаки в UI; ML не пишет альтернативный загрузчик данных в ноутбуке. Ноутбуки допускаются для исследования, но итоговый запуск использует указанные `.py`-модули.
...
2. CSV объединяются по `date`. Из первой таблицы удаляются `Unnamed:*`, из второй — пустой индексный заголовок. Каждый технологический тег получает префикс установки.
...
Внутри приложения — timezone-aware UTC. Исходные даты без timezone интерпретируются согласно `source_timezone`. Начальное значение `Europe/Moscow` является **экспериментальным допущением**, а не установленным свойством пакета; оно записывается в manifest. UI показывает часовой пояс рядом со временем.

- `measured_at` — время измерения/отбора пробы.
- `available_at` — время, начиная с которого результат мог быть известен системе.
- Для телеметрии и ПАК начальное допущение: `available_at = measured_at`.
- Для ЛИМС при отсутствии времени публикации: `available_at = measured_at + lims_delay_hours`. Начальное допущение — 6 часов; отдельно оценить 0, 6 и 12 часов без подбора на финальном тесте.
...
- Для каждого сигнала выполняется выбор последнего доступного измерения по времени. Ближайшее измерение из будущего запрещено.
...
- Устаревший ЛИМС показывается отдельно. Если есть свежий пригодный ПАК, он может стать оперативным источником с предупреждением; устаревший анализ не становится текущей истиной.
...
Типы реализуются в `contracts.py` через Pydantic. Числа должны быть конечными; отсутствие обозначается `None`. Во внешнем JSON — `null`, никогда `NaN`. Неизвестные поля отклоняются. Строковые перечисления сериализуются своими значениями. Агентам передаются новые результаты; они не меняют входной `ProcessState` и конфигурацию.

Обозначения: `Timestamp` — timezone-aware `datetime`; `SignalId`, `CandidateId` и `RunId` — строки; `Unit` — каноническая строка единицы. Таблицы ниже задают обязательные поля; знак `| None` разрешает отсутствие значения, но само поле остаётся в JSON.
...
- Для `hold` обе карты пустые: сохраняются текущие уставки и текущая рецептура. Для `blend` задаётся полная рецептура, а не только изменённые доли. Первая версия не совмещает изменение уставок и рецептуры в одном кандидате.
...
- `MetricEstimate` с `basis='proxy'` не называется физической величиной, если соответствие не установлено. Прокси затрат имеют единицу `proxy_unit`, риска — `index_0_1`.
- `interval_kind='scenario_bound'` означает заданную границу сценария, а не статистическую вероятность. `interval_level` для неё равен `None`.
...
Сигнатуры ниже — спецификация имён и потоков. `PreparedData` и `ModelBundle` — обычные локальные dataclass-контейнеры, а не сетевые DTO. `FeatureFrame` — `pandas.DataFrame` с фиксированным порядком признаков.
...
`predict_quality` и `assess_reliability` оценивают исходное состояние с `candidate_id='hold'`. При оценке действия `effects.py` строит отдельное прогнозное состояние/оценки; исходный объект не изменяется. Данные, рассчитанные моделью, получают соответствующий `basis`, а не `measured`.
...
3. Получить оценки качества и надёжности. При недоступности одной роли сохранить доступные результаты остальных.
...
Полный ключ сортировки: `(risk_index, -throughput, cost_proxy, change_size, candidate_id)`, по возрастанию. Последний элемент стабилизирует выбор при равенстве. Индексы/затраты сравниваются только внутри одного сценария с одинаковыми определениями. Если критерий не обеспечен данными, он **заранее** отключается в конфигурации для всех кандидатов и получает нейтральное место в ключе; это отображается в UI. Пропуск у отдельного кандидата по активному критерию делает его оценку недоступной.

`change_size` для уставок — сумма `abs(new-current)/step`; для рецептуры — сумма абсолютных изменений массовых долей. Порядок приоритетов фиксирован: сначала риск, затем выпуск, затраты, величина изменения.
...
Если `hold` недопустим, порог экономической существенности не мешает выбрать допустимое действие. Начальный cooldown — 60 минут: он подавляет повторную выдачу изменений только при допустимом `hold`. При нарушении качества система повторно предупреждает и оценивает варианты независимо от cooldown. Выдача совета не означает его выполнение: состояние меняется только по новым данным/явно запущенной симуляции.

`DecisionContext` относится к одному сценарию и последовательному воспроизведению. При смене сценария или переходе назад по времени он сбрасывается; иначе обновляется только после `recommend`. Переданный контекст сохраняется в журнале и учитывается при сравнении повторных запусков.
...
- Признаки, отсутствующие во всём обучающем периоде, исключаются. В частности, появление ПАК-плотности в 2025 году не позволяет использовать её как обученный признак модели 2023–2024 без отдельного эксперимента.
- Imputer/scaler и отбор признаков обучаются только на train и сохраняются вместе с моделью. Для Ridge — медианная импутация и масштабирование; для HGB — фиксированная обработка пропусков. Строки с отсутствующей целью не становятся обучающими примерами.
- Для ПАК цель выбирается на сетке `t+60 минут`; при отсутствии точного допустимого наблюдения пример исключается, а цель не переносится из будущего. Для ЛИМС создаётся один пример на реальный анализ: `as_of = measured_at - 60 минут`.
- ПАК и ЛИМС не объединяются в одну безымянную цель. Модель ПАК оценивается по ПАК и отдельно по контрольным ЛИМС; такой перенос источника отражается в отчёте.
...
- Выбор модели выполняется до test: минимизировать MAE при числе пропущенных превышений не больше baseline; при равенстве оставить более простую модель. Если ни одна не удовлетворяет условию — baseline сохраняется, ограничение качества показывается в отчёте.
...
Кандидат, для которого нельзя получить требуемую верхнюю оценку, получает `unknown` по ограничению серы. Обычная ошибка модели не трактуется как вероятность отказа оборудования.
...
Для включения `supports_actions=true` по реальным уставкам необходимы: подтверждённый смысл и управляемость, описанные пределы шага, проверка совместной области применимости, отдельная временная оценка на эпизодах изменений, описанные задержки и ограничения причинной интерпретации. ML сохраняет ссылку на этот отчёт в metadata. До выполнения условий реальные controls остаются отключёнными.

За пределами области применимости модель не экстраполирует ради получения красивой рекомендации. Результат — `unavailable` с причиной. Сравнение прогнозов на истории не выдаётся за доказанный производственный эффект вмешательства.
...
Партия — 100 т; горизонт — 60 минут. Критерий выпуска в этой модели равен массе партии за горизонт и одинаков для всех рецептур. Критерий риска — взвешенный индекс компонентов. Генерируются доли B от 0 до 1 с шагом 0.05; доля A равна `1-B`. Проверки доступности и качества отбрасывают неподходящие варианты. Добавляется отдельный `hold`, дубликаты рецептуры схлопываются в пользу `hold`.

- `blend_normal`: исходная рецептура A=0.9, B=0.1. Сера 8.4; верхняя граница 9.4 мг/кг. Результат — `hold`.
- `blend_risk`: A=0.7, B=0.3. Сера 13.2; верхняя граница 14.2 мг/кг. Результат — A=0.9, B=0.1: это наиболее дешёвая допустимая рецептура на данной сетке.
- `blend_missing`: исходная рецептура как у риска, но сера/граница A отсутствуют. Все потенциально пригодные рецептуры требуют A; результат — `abstain` с объяснением отсутствующих данных.

Для уставок после их подтверждения — до трёх параметров, до пяти значений каждого вокруг текущего режима, не больше 125 комбинаций плюс `hold`. Если конфигурация превышает бюджет, запуск останавливается с понятной ошибкой, а не скрыто обрезает пространство. Реальные шаги и границы фиксируются после анализа источников, без выдуманных температур и давлений в этом документе.
...
Необязательные предупреждения не останавливают весь запуск. При отказе одного кандидата остальные проверяются; при неожиданном исключении агента цикл останавливается, поскольку ошибка реализации не равна недопустимому режиму.
...
`run_id` — UUID; `state_id` — хеш нормализованного состояния. Время и UUID не являются частью критерия численной воспроизводимости. На одинаковых входах, версиях и `DecisionContext` совпадают оценки, выбранный кандидат, статусы и причины.
...
Streamlit rerun сам по себе не запускает новый цикл и не создаёт новые журналы. Расчёт вызывается кнопкой; последний результат хранится в session state. Сохранённые модели и подготовленные данные кэшируются по идентификаторам/хешам. UI не меняет значения, выбранные оптимизатором, и не округляет числа перед проверками.
...
Команды выполняются из корня репозитория после активации Python 3.11 окружения. Они ещё не реализованы на момент принятия дизайна.
...
python -m streamlit run source/ui.py
python -m pytest
python -m ruff check .
python -m ruff format --check .
```

`prepare/train/evaluate` печатают путь созданного результата; `replay/demo` — итог и путь журнала. `abstain` является успешным выполнением расчёта: exit code 0. Ошибки данных/конфигурации/кода — exit code 1. CLI строится на `argparse`; параметры и конфигурация разрешаются один раз в `main.py`. В UI используется тот же загрузчик конфигурации и те же функции.

## 13. Проверки и критерии приёмки

CI работает на малых синтетических fixtures без загрузки 100 МБ LFS и без полного обучения. Полная оценка истории запускается отдельно перед сдачей. Вместо теста `assert True` нужен реальный сквозной `model_demo`.
...
Целевой бюджет после загрузки данных: один цикл с максимум 126 кандидатами до 5 секунд на рабочем CPU-ноутбуке; это инженерная цель, не измеренный результат. В отчёте указать CPU, объём памяти, число кандидатов и фактическое время. Крупное обучение не выполняется при клике в UI.
...
В одной поставке — одна завершённая возможность с проверкой. Автор меняет свою область из раздела 3. Если нужен файл партнёра, передаёт описание изменения владельцу; владение можно временно передать явно, с указанием файлов и commit базы. Одновременное редактирование общего файла не считается способом интеграции.

Backend отвечает за сборку интеграционной версии и координацию слияния. ML проверяет семантику признаков, единиц, ограничений и численных результатов. Проверка партнёром обязательна для общих контрактов и сквозной логики; отдельный третий ревьюер не требуется.

### Первая совместная поставка

До независимой разработки оба исполнителя фиксируют следующие материалы:
...
Backend после этого строит приложение с фиксированными результатами агентов **только в тестовых fixtures**. ML разрабатывает настоящие функции с теми же входами/выходами. В демонстрационном режиме используются реальные формулы сценария, а не заранее записанный успешный ответ.
...
Каждая передача через PR или сообщение содержит одинаковый короткий набор сведений:
...
Модель передаётся не одним `model.joblib`, а полным каталогом из раздела 11, manifest данных/командой их подготовки и примером применения. Локальный абсолютный путь одного исполнителя не является способом передачи: второй должен иметь доступ к артефакту или воспроизводимой команде создания по закреплённому commit. Для первого этапа достаточно небольшой baseline-модели, которую оба могут обучить на CPU.

Передача backend → ML включает подготовленные таблицы, manifest, отчёт проблем и тестовую точку времени. Передача ML → backend включает модель/формулы, metadata, ограничения применимости и ожидаемый результат на этой точке. Получатель воспроизводит результат, а не подтверждает только получение файла.

### Регулярная интеграция

Не реже одного раза в рабочий день, а также после изменения контракта:
...
3. Оба запускают три модельных сценария и проверяют `hold/recommend/abstain`. При наличии модели дополнительно проверяется её загрузка и одна общая историческая точка.
4. Сравниваются выбранные источники, возраст анализов, список признаков, оценки, ограничения и итог; расхождение чисел разбирается до нового улучшения.
...
Незавершённая возможность обозначается через capabilities и понятный отказ, не скрывается условной константой. Backend может реализовать весь цикл на зафиксированных синтетических сценариях, пока ML проверяет реальные соответствия и модели.

Что должно быть установлено на этапе 0 и не подменяется архитектурным решением: истинные единицы спорных тегов, управляющие параметры гидроочистки, технологические пределы, подтверждённые связи потоков, задержки, реальная товарная спецификация сверх серы. До подтверждения соответствующие действия и заявления о полноте проверки отключены. Владелец исследования — ML; backend обеспечивает исполнение запрета.
