...
> Проверенная поставка и материалы жюри: [GitHub Releases](https://github.com/bug00n/HackathonPetrolCode/releases/latest). Целевая архитектура ниже не означает наличие промышленного управления. Планы этапов и журнал разработки сохранены только в ветке dev.

### Уточнения реализованного контракта снимка и UI

`ProcessState` — ограниченный снимок, не копия всей истории. После фильтрации `measured_at <= as_of` и `available_at <= as_of` для каждого сигнала и источника сохраняются последнее наблюдение и последнее валидное ненулевое (если отличаются). Здесь «ненулевое» означает не `null`; численный ноль остаётся допустимым измерением. Последняя невалидная запись не скрывает предыдущее пригодное значение. Выбор свежести и приоритета источников сохраняется; полная история доступна в prepared dataset и используется при расчёте признаков. Журнал содержит этот воспроизводимый снимок, а не весь dataset.

Фоновые задачи не вызывают Tcl/Tk: результаты и прогресс идут через очередь главному UI-потоку. Исторические формы разделяют dataset и время. Интервал загружает данные и модель один раз, отменяется между точками и возвращает явно помеченный частичный результат. Экспорт относится к показанному результату. В Windows-поставке ресурсы определяются относительно EXE; каталог `_internal` и закреплённые данные должны оставаться рядом. Управление оборудованием по-прежнему отключено.
...
Основания: [техническое задание](materials/ТЗ_нефтекод.docx), [материалы](materials/README.md), [поэтапный план](https://github.com/bug00n/HackathonPetrolCode/blob/dev/IMPLEMENTATION_PLAN.md). ТЗ определяет обязательные требования, этот документ — технические контракты, план — порядок реализации. При изменении контракта документ и тестовые примеры обновляются в том же PR.
...
ресурсов команде. Основания и открытые вопросы: [разбор встречи](docs/technical/QA_CLARIFICATIONS.md).
...
Подробная карта и границы: [разбор схем](docs/technical/SCHEME_ANALYSIS_2026_09_14.md).
...
Редактор `source/ui_what_if.py` создаёт проверенную копию
`ScenarioConfig` только для `model_demo` без реальных controls. Свойства A/B,
запасы, доли, масса партии и синтетическая кривая присадки передаются через
`run_model_demo(scenario_override=...)` существующему циклу решения.
Preset на диске не изменяется; изменённые входы попадают в журнал и JSON-экспорт.
Сброс восстанавливает поля preset и требует явного пересчёта.
С v1.2.0 он подбирает допустимую рецептуру двух компонентов при закреплённых
значениях долей или присадки. Предложения проходят тот же фильтр ограничений,
что и обычный расчёт; исходное и показанное значения записываются в журнал.

`source/ui_charts.py` строит график исторического point/upper и лимита серы.
Ось времени соответствует цели прогноза (as_of + 60 минут); пропуски сохраняются,
неизвестные значения не подменяются нулями. Пути данных и моделей на странице
истории находятся в свёрнутых дополнительных настройках. Ни редактор, ни график
не меняют `supports_actions=false`.
...
> **Актуальная семантика:** уточнения организаторов от 15.09.2026 и текущий
> `config/tags.csv` заменяют прежние спорные трактовки коротких имён тегов.
> Для текущего release использовать только исправленный словарь и его хеш в
> manifest; старые описания сохраняются как архивный контекст и не определяют
> runtime-связи P8/F19/T11.
...
**Оговорка после Q&A 11.09:** в [вопросах участников](docs/technical/QA_CLARIFICATIONS.md) оспорены
